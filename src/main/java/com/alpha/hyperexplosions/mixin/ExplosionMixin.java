		...
		if(INSTANCE.getConfig().modEnabled) {
			if(!isUnderWater) {
				...
				if(INSTANCE.getConfig().showBlastWave) {
					world.addParticle(HyperExplosions.BLASTWAVE, x, y, z, 0, 0, 0);
				}
				if(INSTANCE.getConfig().showFireball) {
					world.addParticle(HyperExplosions.FIREBALL, x, y + 0.5, z, 0, 0, 0);
				} else if (INSTANCE.getConfig().showSparks) {
					world.addParticle(HyperExplosions.BLANK_FIREBALL, x, y + 0.5, z, 0, 0, 0);
				}
				if(INSTANCE.getConfig().showMushroomCloud) {
					...
					world.addParticle(HyperExplosions.SMOKE, x, y, z, 0, 0.15, 0);
					world.addParticle(HyperExplosions.SMOKE, x, y, z, 0, 0.4, 0);
					world.addParticle(HyperExplosions.SMOKE, x, y, z, 0.15, 0.4, 0);
					world.addParticle(HyperExplosions.SMOKE, x, y, z, 0, 0.4, 0.15);
					world.addParticle(HyperExplosions.SMOKE, x, y, z, -0.15, 0.4, 0);
					world.addParticle(HyperExplosions.SMOKE, x, y, z, 0, 0.4, -0.15);
				}
				...
				if(INSTANCE.getConfig().showShockwave) {
					world.addParticle(HyperExplosions.SHOCKWAVE, x, y + 0.5, z, 0, 0, 0);
				} else if (INSTANCE.getConfig().showUnderwaterSparks) {
					world.addParticle(HyperExplosions.BLANK_SHOCKWAVE, x, y + 0.5, z, 0, 0, 0);
				}
				if(INSTANCE.getConfig().showUnderwaterBlastWave) {
					world.addParticle(HyperExplosions.UNDERWATERBLASTWAVE, x, y + 0.5, z, 0, 0, 0);
				}
				for(int total = INSTANCE.getConfig().bubbleAmount; total >= 1; total--) {
					world.addParticle(HyperExplosions.BUBBLE, x, y, z, this.random.nextBetween(1, 7) * 0.3 * this.random.nextBetween(-1, 1), this.random.nextBetween(1, 10) * 0.1, this.random.nextBetween(1, 7) * 0.3 * this.random.nextBetween(-1, 1));
				}
